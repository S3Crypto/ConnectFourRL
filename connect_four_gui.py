...
class ConnectFourGUI:
    def __init__(self, root):
        ...
        # Load the RL model
        self.model = PPO.load("connect_four_ppo")
    ...
    def create_board(self):
        ...
        # Create the game board grid with initially white cells
    ...
    def handle_move(self, column):
        if self.game.drop_disc(column):
            ...
            if self.game.is_winning_move(self.game.current_player):
                ...
                self.ai_move()  # Let the AI make a move after the human

    def ai_move(self):
        # Flatten the board for the model input and predict the best action
        obs = self.game.board.flatten()
        action, _ = self.model.predict(obs)
        self.game.drop_disc(action)
        self.update_display()
        if self.game.is_winning_move(self.game.current_player):
            messagebox.showinfo("Connect Four", f"AI (Player {self.game.current_player}) wins!")
            self.game.update_score(self.game.current_player)
            self.update_scoreboard()
            self.reset_board()
        elif self.game.is_full():
